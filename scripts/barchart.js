var dataFromJSONBarChart = '';
var counterBarChart = [0, 0, 0, 0, 0];
'use strict';
...
function readTextFileBarChart(file) {
  ...
  rawFile.onreadystatechange = function () {
    if (rawFile.readyState === 4) {
      if (rawFile.status === 200 || rawFile.status == 0) {
        ...
        dataFromJSONBarChart = JSON.parse(allText);
      }
    }
  }
  rawFile.send(dataFromJSONBarChart);
}

readTextFileBarChart("scripts/lungime_poze.json");


for (i = 0; i < dataFromJSONBarChart.length; ++i) {
  if (dataFromJSONBarChart[i] < 360001)
    counterBarChart[0]++;
  if (dataFromJSONBarChart[i] < 691200 && dataFromJSONBarChart[i] > 360000)
    counterBarChart[1]++;
  if (dataFromJSONBarChart[i] < 810000 && dataFromJSONBarChart[i] > 691200)
    counterBarChart[2]++;
  if (dataFromJSONBarChart[i] < 1166401 && dataFromJSONBarChart[i] > 810000)
    counterBarChart[3]++;
  if (dataFromJSONBarChart[i] > 1166401)
    counterBarChart[4]++;
}
...
window.onload = function () {
  ...
  // Chart Data
  var data = [
    { label: 'less than 3260001', value: counterBarChart[0] },
    { label: 'between 360000 and 691200', value: counterBarChart[1] },
    { label: 'between 691200 and 810000', value: counterBarChart[2] },
    { label: 'between 810000 and 1166401', value: counterBarChart[3] },
    { label: 'more than 1166401', value: counterBarChart[4] },
  ];
